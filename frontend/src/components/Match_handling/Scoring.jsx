...
function Scoring() {
  ...
  function score(value) {
    ...
    if (batting == 1) {
      if (match) {
        if (!match.commentary) {
          match.commentary = []; // Initialize commentary as an empty array if it's undefined
        }

        match.commentary[0] = match.commentary[0] || [];
        match.commentary[0].push(
          batsman1.name + " to " + bowler.name + " " + value + " run"
        );
      ...
      if (match) {
        ...
        match.commentary[1] = match.commentary[1] || [];
  ...
  const handleWicket = (event) => {
    ...
    if (batting == 1) {
      ...
      match.commentary[0] = match.commentary[0] || [];
      match.commentary[0].push(bowler.name + " take wicket of " + batsman1.name);
      ...
      match.commentary[1] = match.commentary[1] || [];
      match.commentary[1].push(bowler.name + " take wicket of " + batsman1.name);
    ...
    if (batting == 1) {
      if (match.wicket[0] != 10) setiswicket(1);
      if (match.wicket[0] == 10) {
        handleInningend();
      }
    } else {
      if (match.wicket[1] != 10) setiswicket(1);
  ...
  const handleWide = (event) => {
    ...
    if (batting == 1) {
      ...
      match.commentary[0] = match.commentary[0] || [];
      if (match) {
        match.commentary[0].push(batsman1.name + " to " + bowler.name + " wide");
        if (match.score[0] == null) {
          match.score.push(0);
        }
      ...
      match.commentary[1] = match.commentary[1] || [];
      match.commentary[1].push(batsman1.name + " to " + bowler.name + " wide");
  ...
  const handleInningend = async (event) => {
    if (batting == 1) {
      if (
        match.over[0] == parseFloat(match.total_over) ||
        match.wicket[0] == 10
      ) {
        if (!match.commentary) {
          match.commentary = []; // Initialize commentary as an empty array if it's undefined
      ...
      match.over[1] == parseFloat(match.total_over) ||
      match.wicket[1] == 10 ||
